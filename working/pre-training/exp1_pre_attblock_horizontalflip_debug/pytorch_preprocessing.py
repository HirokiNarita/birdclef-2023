...
class BirdDataset(Dataset):
    ...
    def torchaudio_read(self, filepath, offset=None):
        if offset is not None:
            audio_org, sr = torchaudio.load(filepath, frame_offset=offset*self.sr, num_frames=self.audio_length)
        else:
        ...
        # to numpy
        audio_org = audio_org.squeeze().numpy().copy()
        return audio_org
    ...
    def read_file(self, filepath):
        ...
        if self.is_train == True:
            ...
            if self.is_test == True:
                ...
                #audio_org = self.torchaudio_read(filepath)
            else:
                # for valid
                audio_org, sr = lb.load(filepath, offset=0)
                #audio_org = self.torchaudio_read(filepath, offset=0)
        # train
        if self.is_train == True:
