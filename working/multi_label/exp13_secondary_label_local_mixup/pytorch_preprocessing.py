...
from audio_augment import OneOf, GaussianNoiseSNR, PinkNoiseSNR
...
class BirdDataset(Dataset):
    def __init__(self,
        ...
        self.mix_duration = CFG.mix_duration
        self.mix_length = self.mix_duration*self.sr
        
        self.audio_length = self.duration*self.sr
        self.step = step or self.audio_length
        
        # self.add_noise = OneOf([
        #     GaussianNoiseSNR(min_snr=10,max_snr=20,p=0.5),
        #     PinkNoiseSNR(min_snr=10,max_snr=20,p=0.5)])
    ...
    def local_wav_mixup(self, audio_org):
        n = int(self.mix_length / self.audio_length)
        mix_audio = np.zeros((self.audio_length))
        for i in range(n):
            start_idx = i*self.audio_length
            end_idx = (i+1)*self.audio_length
            part = audio_org[start_idx: end_idx]
            mix_audio = mix_audio + part
    ...
    def read_file(self, filepath):
        ...
        if self.is_train == True:
            offset, audio_length = self.get_audio_offset(filepath, duration=self.duration)
            if audio_length > self.audio_length:
                audio_org, sr = lb.load(filepath, offset=offset, duration=self.mix_duration, sr=self.sr, mono=True)
                audio_org = self.local_wav_mixup(audio_org)
            else:
                audio_org, sr = lb.load(filepath, offset=offset, duration=self.duration, sr=self.sr, mono=True)
            # augment
            #audio_org = self.add_noise(audio_org)
        else:
            if self.is_test == True:
                # for test
    ...
    def make_multi_onehot(self, multi_label_names):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_test == False:
            sample_info['multi_label_target'] = self.make_multi_onehot(sample_info['multi_labels'])
